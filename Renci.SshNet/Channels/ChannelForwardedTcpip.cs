...
namespace Renci.SshNet.Channels
{
    ...
    internal partial class ChannelForwardedTcpip : Channel
    {
        ...
        public void Bind(IPAddress connectedHost, uint connectedPort)
        {
            ...
            ServerWindowSize = LocalWindowSize;
            ...
            try
            {
                ...
                buffer = new byte[PacketSize - 9];

                OpenSocket(connectedHost, connectedPort);

                //  Send channel open confirmation message
                SendMessage(new ChannelOpenConfirmationMessage(RemoteChannelNumber, LocalWindowSize, PacketSize,
                    LocalChannelNumber));
            }
            catch (Exception exp)
            {
