namespace Renci.SshNet.Security.Cryptography
{
    ...
    public abstract class Cipher
    {
        ...
        /// </summary>
        /// <param name="buffer">The buffer.</param>
        /// <returns>Converted <see cref="int" />.</returns>
        protected static uint BigEndianToUInt32(byte[] buffer)
        {
            var n = (uint) buffer[0] << 24;
            n |= (uint) buffer[1] << 16;
            n |= (uint) buffer[2] << 8;
            n |= buffer[3];
            return n;
        ...
        {
            var hi = BigEndianToUInt32(buffer);
            var lo = BigEndianToUInt32(buffer, 4);
        ...
        {
            var hi = BigEndianToUInt32(buffer, offset);
            var lo = BigEndianToUInt32(buffer, offset + 4);
            return ((ulong) hi << 32) | lo;
        }
        ...
        {
            var lo = LittleEndianToUInt32(buffer);
            var hi = LittleEndianToUInt32(buffer, 4);
            return ((ulong) hi << 32) | lo;
        }
        ...
        {
            var lo = LittleEndianToUInt32(buffer, offset);
            var hi = LittleEndianToUInt32(buffer, offset + 4);
            return ((ulong) hi << 32) | lo;
        }
