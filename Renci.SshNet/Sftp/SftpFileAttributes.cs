...
namespace Renci.SshNet.Sftp
{
    ...
    public class SftpFileAttributes
    {
        ...
        internal SftpFileAttributes(DateTime lastAccessTime, DateTime lastWriteTime, long size, int userId, int groupId,
            uint permissions, IDictionary<string, string> extensions)
        {
        ...
        internal bool IsExtensionsChanged
        {
            get
            {
                return _originalExtensions != null && Extensions != null &&
                       !_originalExtensions.SequenceEqual(Extensions);
            }
        }
        ...
        private static readonly uint S_IFMT = 0xF000; //  bitmask for the file type bitfields

        private static readonly uint S_IFSOCK = 0xC000; //	socket

        private static readonly uint S_IFLNK = 0xA000; //	symbolic link

        private static readonly uint S_IFREG = 0x8000; //	regular file

        private static readonly uint S_IFBLK = 0x6000; //	block device

        private static readonly uint S_IFDIR = 0x4000; //	directory

        private static readonly uint S_IFCHR = 0x2000; //	character device

        private static readonly uint S_IFIFO = 0x1000; //	FIFO

        private static readonly uint S_ISUID = 0x0800; //	set UID bit

        private static readonly uint S_ISGID = 0x0400; //	set-group-ID bit (see below)

        private static readonly uint S_ISVTX = 0x0200; //	sticky bit (see below)

        private static readonly uint S_IRUSR = 0x0100; //	owner has read permission

        private static readonly uint S_IWUSR = 0x0080; //	owner has write permission

        private static readonly uint S_IXUSR = 0x0040; //	owner has execute permission

        private static readonly uint S_IRGRP = 0x0020; //	group has read permission

        private static readonly uint S_IWGRP = 0x0010; //	group has write permission

        private static readonly uint S_IXGRP = 0x0008; //	group has execute permission

        private static readonly uint S_IROTH = 0x0004; //	others have read permission

        private static readonly uint S_IWOTH = 0x0002; //	others have write permission

        private static readonly uint S_IXOTH = 0x0001; //	others have execute permission
