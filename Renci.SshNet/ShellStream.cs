...
namespace Renci.SshNet
{
    ...
    public partial class ShellStream : Stream
    {
        ...
        internal ShellStream(Session session, string terminalName, uint columns, uint rows, uint width, uint height,
            int maxLines, IDictionary<TerminalModes, uint> terminalModeValues)
        {
        ...
        /// <param name="expectActions">
        ///     The expected expressions and actions to perform, if the specified time elapsed and expected
        ///     condition have not met, that method will exit without executing any action.
        /// </param>
        ...
        /// <param name="timeout">Time to wait for input.</param>
        /// <returns>
        ///     Text available in the shell that contains all the text that ends with expected expression, if the specified time
        ///     elapsed returns null.
        /// </returns>
            ...
            while (true)
            {
                lock (_incoming)
                {
                    ...
                    if (index >= 0)
                    {
                        ...
                        //  Remove processed items from the queue
                        for (var i = 0; i < index + 2 && _incoming.Count > 0; i++)
                        {
                            _incoming.Dequeue();
                        }
                        break;
